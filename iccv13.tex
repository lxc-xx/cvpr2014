...
\usepackage{iccv2013authorKit_latex/iccv}
\usepackage{times}
\usepackage{epsfig}
\usepackage{graphicx}
\usepackage{amsmath}
...
\def\iccvPaperID{652} % *** Enter the CVPR Paper ID here
...
% in terms of histograms of optical flow (HOF)~\cite{Laptev08} and motion boundary histograms (MBH)~\cite{Wang12}.
%Motion is an important cue used in common video descriptors for action recognition such as  histograms of optical flow (HOF)~\cite{Laptev08} and motion boundary histograms (MBH)~\cite{Wang12}.
...
We evaluate the speed and accuracy of our approach on \mbox{Hollywood-2}~\cite{Marszalek09}, UCF50~\cite{Reddy12} and UT-Interaction~\cite{Ryoo10} benchmarks. %action recognition benchmarks.
...
%Recognition of events and human actions in video has been in focus of computer vision for long time. 
%Action recognition is an active research area. 
...
%In particular, MBH descriptors computed along point trajectories have recently improved accuracy of action recognition in a number of challenging datasets~\cite{Wang12}. We follow this work and design a new motion-based local descriptor that drastically improves the speed of previous methods at the cost of minor decrease in the recognition accuracy.
...
%while approximating the distance in original space (including many variants of hashing) is given by \cite{Grauman13}.
...
%Meanwhile, we can observe a rather high level of noise for all three flow estimates.
...
\label{sec:dimred}
The FV encoding of a single video using all types of descriptors and grids in this paper has about $2,5\times10^6$ elements and takes $10\text{Mb}$ of RAM in our implementation. Such high dimensionality implies scalability problems due to large memory requirements. % , which is quite large and makes linear kernel calculation a difficult technical challenge by itself, because realistic datasets like (Hollywood2, UCF50, etc) count thousands of videos, and fitting all the Fisher vectors in memory becomes not feasible.
...
%  exist that aim to the dot product in the original space. 
...
%We have also tried the supervised metric learning formulation of \cite{Mensink12} using the code obtained from the authors. However, it did not improve the result, and we focus on evaluating the PCA approach. First, we train PCA matrices separately for every one of the 96 channels. Then we reduce dimension of every channel, concatenate them back without renormalization, and train another PCA matrix that captures correlation between different descriptor types. We discuss the trade-off between the resulting dimension and accuracy in the next section.
...
{
\small
\bibliographystyle{iccv2013authorKit_latex/ieee}
\bibliography{shortstrings,iccv13}
}
